...
<div class="wc-order-data-row wc-order-totals-items wc-order-items-editable">
	...
	<table class="wc-order-totals">
			<tr>
				<td class="label"><?php esc_html_e( 'Tổng phụ mục:', 'woocommerce' ); ?></td>
				<td width="1%"></td>
				<td class="total">
		...
		<?php if ( 0 < $order->get_total_discount() ) : ?>
			<tr>
				<td class="label"><?php esc_html_e( 'Phiếu mua hàng(s):', 'woocommerce' ); ?></td>
				<td width="1%"></td>
				<td class="total">-
		...
		<?php if ( 0 < $order->get_total_fees() ) : ?>
			<tr>
				<td class="label"><?php esc_html_e( 'Phí:', 'woocommerce' ); ?></td>
				<td width="1%"></td>
				<td class="total">
		...
		<?php if ( $order->get_shipping_methods() ) : ?>
			<tr>
				<td class="label"><?php esc_html_e( 'Đang vận chuyển:', 'woocommerce' ); ?></td>
				<td width="1%"></td>
				<td class="total">
		...
		<tr>
			<td class="label"><?php esc_html_e( 'Tổng đơn hàng', 'woocommerce' ); ?>:</td>
			<td width="1%"></td>
			<td class="total">
	...
	<?php if ( in_array( $order->get_status(), array( 'processing', 'completed', 'refunded' ), true ) && ! empty( $order->get_date_paid() ) ) : ?>
		...
		<table class="wc-order-totals" style="border-top: 1px solid #999; margin-top:12px; padding-top:12px">
			<tr>
				<td class="<?php echo $order->get_total_refunded() ? 'label' : 'label label-highlight'; ?>"><?php esc_html_e( 'Đã thanh toán', 'woocommerce' ); ?>: <br /></td>
				<td width="1%"></td>
				<td class="total">
					<?php echo wc_price( $order->get_total(), array( 'currency' => $order->get_currency() ) ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
				</td>
	...
	<?php if ( $order->get_total_refunded() ) : ?>
		<table class="wc-order-totals" style="border-top: 1px solid #999; margin-top:12px; padding-top:12px">
			<tr>
				<td class="label refunded-total"><?php esc_html_e( 'Hoãn lại', 'woocommerce' ); ?>:</td>
				<td width="1%"></td>
...
<div class="wc-order-data-row wc-order-bulk-actions wc-order-data-row-toggle">
	<p class="add-items">
		<?php if ( $order->is_editable() ) : ?>
			<button type="button" class="button add-line-item"><?php esc_html_e( 'Thêm sản phẩm(s)', 'woocommerce' ); ?></button>
			<?php if ( wc_coupons_enabled() ) : ?>
				<button type="button" class="button add-coupon"><?php esc_html_e( 'Phiếu giảm giá', 'woocommerce' ); ?></button>
			<?php endif; ?>
		<?php else : ?>
			<span class="description"><?php echo wc_help_tip( __( 'Để chỉnh sửa đơn đặt hàng này, hãy thay đổi trạng thái trở lại "Chờ xử lý"', 'woocommerce' ) ); ?> <?php esc_html_e( 'This order is no longer editable.', 'woocommerce' ); ?></span>
		<?php endif; ?>
		<?php if ( 0 < $order->get_total() - $order->get_total_refunded() || 0 < absint( $order->get_item_count() - $order->get_item_count_refunded() ) ) : ?>
			<button type="button" class="button refund-items"><?php esc_html_e( 'Hoàn tiền', 'woocommerce' ); ?></button>
		<?php endif; ?>
		<?php
		...
		?>
		<?php if ( $order->is_editable() ) : ?>
			<button type="button" class="button button-primary calculate-action"><?php esc_html_e( 'Tính toán lại', 'woocommerce' ); ?></button>
		<?php endif; ?>
	</p>
</div>
