...
class WC_Query {
	...
	public function get_endpoint_title( $endpoint, $action = '' ) {
		...
		switch ( $endpoint ) {
			case 'order-pay':
				$title = __( 'Trả tiền đặt hàng', 'woocommerce' );
				break;
			case 'order-received':
				$title = __( 'Đơn hàng đã nhận', 'woocommerce' );
				break;
			case 'orders':
				if ( ! empty( $wp->query_vars['orders'] ) ) {
					/* translators: %s: page */
					$title = sprintf( __( 'Đơn hàng (page %d)', 'woocommerce' ), intval( $wp->query_vars['orders'] ) );
				} else {
					$title = __( 'Đơn hàng', 'woocommerce' );
				}
				break;
			case 'view-order':
				$order = wc_get_order( $wp->query_vars['view-order'] );
				/* translators: %s: order number */
				$title = ( $order ) ? sprintf( __( 'Đơn hàng #%s', 'woocommerce' ), $order->get_order_number() ) : '';
				break;
			case 'downloads':
				$title = __( 'Tải xuống', 'woocommerce' );
				break;
			case 'edit-account':
				$title = __( 'Chi tiết tài khoản', 'woocommerce' );
				break;
			case 'edit-address':
				$title = __( 'Địa chỉ', 'woocommerce' );
				break;
			case 'payment-methods':
				$title = __( 'Phương thức thanh toán', 'woocommerce' );
				break;
			case 'add-payment-method':
				$title = __( 'Thêm phương thức thanh toán', 'woocommerce' );
				break;
			case 'lost-password':
				if ( in_array( $action, array( 'rp', 'resetpass', 'newaccount' ) ) ) {
					$title = __( 'Đặt mật khẩu', 'woocommerce' );
				} else {
					$title = __( 'Quên mật khẩu', 'woocommerce' );
				}
				break;
			default:
				$title = '';
