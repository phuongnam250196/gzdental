...
if ( ! class_exists( 'WC_Email_Customer_Refunded_Order', false ) ) :
	...
	class WC_Email_Customer_Refunded_Order extends WC_Email {
		...
		public function get_default_subject( $partial = false ) {
			if ( $partial ) {
				return __( 'Đơn hàng #{order_number} của bạn đã được hoàn lại 1 phần', 'woocommerce' );
			} else {
				return __( 'Đơn hàng #{order_number} của bạn đã được hoàn lại tiền', 'woocommerce' );
			}
		}
		...
		public function get_default_heading( $partial = false ) {
			if ( $partial ) {
				return __( 'Hoàn lại một phần: Đơn hàng	{order_number}', 'woocommerce' );
			} else {
				return __( 'Đơn hàng đã được hoàn lại: {order_number}', 'woocommerce' );
			}
		}
		...
		public function get_default_additional_content() {
			return __( 'Mong rằng chúng tôi sẽ sớm gặp lại bạn.', 'woocommerce' );
		}
