...
function wc_template_redirect() {
	...
	if ( is_page( wc_get_page_id( 'checkout' ) ) && wc_get_page_id( 'checkout' ) !== wc_get_page_id( 'cart' ) && WC()->cart->is_empty() && empty( $wp->query_vars['order-pay'] ) && ! isset( $wp->query_vars['order-received'] ) && ! is_customize_preview() && apply_filters( 'woocommerce_checkout_redirect_empty_cart', true ) ) {
		wc_add_notice( __( 'Thanh toán không có sẵn trong khi giỏ hàng của bạn trống.', 'woocommerce' ), 'notice' );
...
if ( ! function_exists( 'woocommerce_widget_shopping_cart_subtotal' ) ) {
	...
	function woocommerce_widget_shopping_cart_subtotal() {
		echo '<strong>' . esc_html__( 'Tổng phụ:', 'woocommerce' ) . '</strong> ' . WC()->cart->get_cart_subtotal(); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
	}
}
	...
	function woocommerce_breadcrumb( $args = array() ) {
		$args = wp_parse_args(
			...
			apply_filters(
				...
				array(
					'delimiter'   => '&nbsp;&#47;&nbsp;',
					'wrap_before' => '<div class="breadcrumbs"><div class="container"><div class="inner"><ul>',
...
if ( ! function_exists( 'woocommerce_form_field' ) ) {
	...
	function woocommerce_form_field( $key, $args, $value = null ) {
		...
		if ( $args['required'] ) {
			...
			$required        = '&nbsp;<abbr class="required" title="' . esc_attr__( 'Bắt buộc', 'woocommerce' ) . '">*</abbr>';
		} else {
			$required = '&nbsp;<span class="optional">(' . esc_html__( 'Không bắt buộc', 'woocommerce' ) . ')</span>';
		}
		...
		switch ( $args['type'] ) {
			case 'country':
				...
				if ( 1 === count( $countries ) ) {
					...
					$field = '<select name="' . esc_attr( $key ) . '" id="' . esc_attr( $args['id'] ) . '" class="country_to_state country_select ' . esc_attr( implode( ' ', $args['input_class'] ) ) . '" ' . implode( ' ', $custom_attributes ) . ' data-placeholder="' . esc_attr( $args['placeholder'] ? $args['placeholder'] : esc_attr__( 'Chọn quốc gia/ khu vực&hellip;', 'woocommerce' ) ) . '"><option value="">' . esc_html__( 'Select a country / region&hellip;', 'woocommerce' ) . '</option>';
					...
					$field .= '<noscript><button type="submit" name="woocommerce_checkout_update_totals" value="' . esc_attr__( 'Update country / region', 'woocommerce' ) . '">' . esc_html__( 'Cập nhật quốc gia/ khu vực', 'woocommerce' ) . '</button></noscript>';
...
function wc_empty_cart_message() {
	echo '<p class="cart-empty woocommerce-info">' . wp_kses_post( apply_filters( 'wc_empty_cart_message', __( 'Giỏ hàng của bạn hiện đang trống.', 'woocommerce' ) ) ) . '</p>';
}
