	...
	<div class="add-to-cart">
		<form class="cart grouped_form" action="<?php echo esc_url( apply_filters( 'woocommerce_add_to_cart_form_action', $product->get_permalink() ) ); ?>" method="post" enctype='multipart/form-data'>
							...
							switch ( $column_id ) {
								case 'quantity':
									...
									if ( ! $grouped_product_child->is_purchasable() || $grouped_product_child->has_options() || ! $grouped_product_child->is_in_stock() ) {
										...
										echo '<input type="checkbox" id="quantity_'.$grouped_product_child->get_id().'" name="' . esc_attr( 'quantity[' . $grouped_product_child->get_id() . ']' ) . '" value="1" class="wc-grouped-product-add-to-cart-checkbox" />';
									} else {
										?>
										<div class="pull-left qty-number">
											<div class="custom pull-left">
											<button
											onclick="var result = document.getElementById('quantity_<?php echo $grouped_product_child->get_id(); ?>'); console.log(result); var qty = result.value; if( !isNaN( qty ) ) result.value--;return false;" class="reduced items-count" type="button"><i
											...
											woocommerce_quantity_input(
												array(
													'input_id' => 'quantity_' . $grouped_product_child->get_id(),
											...
											?>
											<button onclick="var result = document.getElementById('quantity_<?php echo $grouped_product_child->get_id(); ?>'); var qty = result.value; if( !isNaN( qty )) result.value++;return false;"
			...
			<?php if ( $quantites_required && $show_add_to_cart_button ) : ?>
				...
				<button type="submit" class="single_add_to_cart_button button alt btn-cart"><?php echo esc_html( $product->single_add_to_cart_text() ); ?></button>
